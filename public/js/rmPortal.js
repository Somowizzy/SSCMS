...
async function renderRmDash() {
  ...
  try {
    ...
    const greeting = (typeof greeting === 'function' ? greeting() : (new Date().getHours() < 12 ? 'morning' : new Date().getHours() < 17 ? 'afternoon' : 'evening'));
    ...
    setHTML('#page-content', `
      ...
      <div style="font-size:14px;color:var(--txt2);margin-bottom:2px">
        Good ${greeting}, <strong style="color:var(--txt)">${esc(firstName)}</strong> &mdash; ${dateStr}
      </div>
